<script lang="ts">
    ...
    const loadRecentFeedbackFunction = async (): Promise<Array<object | null>> => {
...
{#if postSelected === null}
    ...
    <Tabs available_tabs={["Posts", "Feedback"]} bind:selected_tab={tabSelected}/>
    {#if tabSelected === "Posts"}
        ...
        {:then data}
            ...
            {:else}
                <button class="bg-white rounded-md p-2 inline-flex items-center justify-center text-gray-400 hover:text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-indigo-500 text-lg"
                        on:click={createNewPost}>
                                ...
                                <table class="min-w-full">
                                    ...
                                    {#each data as post}
                                        <tr class="bg-white border-b cursor-pointer"
                                            on:click={() => {postSelected= post.slug}}>
                        ...
                        <div class="overflow-hidden">
                            <table class="min-w-full">
                                ...
                                {#each data as feedback}
                                    <tr class="bg-white border-b cursor-pointer"
                                        on:click={() => {feedbackPopUp = feedback}}>
                                        <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                                            {#if feedback.thumbs_up}
                                                <!-- heroicons/thumb-up -->
                                                <svg class="inline-block h-8 w-8 align-middle rounded-full bg-green-600"
                                            ...
                                            {:else}
                                                ...
                                                <svg class="inline-block h-8 w-8 align-middle rounded-full bg-red-600"
...
{#if feedbackPopUp !== null}
    ...
    <div class="fixed top-0 left-0 w-screen h-screen z-10 bg-white">
        <button on:click={() => {feedbackPopUp = null}}>Close</button>
        <p>Text:</p>
