...
pub async fn upload_file(
    ...
    let res = web::block(move || {
        let conn = pool.get()?;
        actions::add_file(&data.into_inner(), &conn)
    })
        .await?
...
pub async fn get_file(
    ...
    let data = web::block(move || {
        let conn = pool.get()?;
        actions::get_file(query.into_inner(), &conn)
    })
        .await?
...
pub async fn delete_file(
    ...
    web::block(move || {
        let conn = pool.get()?;
        actions::delete_file(query.into_inner(), &conn)
    })
        .await?
        .map_err(actix_web::error::ErrorNotFound)?;
...
pub async fn list_files(
    ...
    let files = web::block(move || {
        let conn = pool.get()?;
        actions::get_all_files(&offset.offset, &conn)
    })
        .await?
